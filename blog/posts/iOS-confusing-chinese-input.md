...
用户反馈在 iOS 上不能输入中文，输入拼音后点击候选词无法填入，且输入框内拼音会重复叠加。

经排查，问题原因如下：

1. 长度限制：输入框最大长度为 6。iOS 原生键盘在中文输入未选词时会先将拼音填入，当拼音长度超过 6 时，选词功能失效。

2. 字符叠加：输入过程中存在内容重复问题，如输入 "gh" 时，由于频繁触发 onChange 事件剔除掉空格，导致输入框错误地显示为 "ghgh"。这又加剧了选词功能失效的问题。

所以要解决的问题是为什么在输入拼音时，过滤空格会导致异常？以及 onChange 事件会频繁触发？
...
参考 @hec9527 的回答：https://segmentfault.com/q/1010000018185771
